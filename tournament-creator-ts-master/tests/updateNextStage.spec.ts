...
describe('updateNextStage grp->se', () => {
  it('should update next stage correctly for grp1->se1', () => {
    ...
    expect(TestHelperFunctions.checkUpdateNextStageResult(tournament, 0, result)).toBeTruthy();
  });
...
describe('updateNextStage se->se', () => {
  it('should update next phase correctly for grp2->grp4 + thirdPlaceMatch', () => {
    let stages: stageParams[] = [
      {
        nrGroups: 2,
        nrParticipants: 4,
    ...
    let result = [
      [
        ...
        { 'token': '01aA0', 'advantage': 1, 'username': 'A1' }
      ],
      [
        { 'token': '02aA1', 'advantage': 0, 'username': 'B0' },
        { 'token': '01aA1', 'advantage': 0, 'username': 'B1' }
      ]
    ];
  ...
  it('should update next phase correctly with 6 participants for grp4->grp2', () => {
    ...
    let result = [
      ...
      [
        { 'token': '01aB1', 'advantage': 0, 'username': MatchFactory.EMPTY_ENTRY.token }
      ]
    ];
  ...
  it('should update a full single elimination tournament correctly with nrGroup=4', () => {
    ...
    for(let phase=0; phase<maxPhase; phase++) {
      check = TestHelperFunctions.checkUpdateNextStageResult(tournament, phase, result[phase]);
      if (!check) break;
    }
    ...
    let result = [
      ...
      [
        [ // Finale und Spiel um Platz 3
          { 'token': '04aA0', 'advantage': 1, 'username': 'A0' },
          { 'token': '03aA0', 'advantage': 1, 'username': 'A1' }
        ],
        [
          { 'token': '04aA1', 'advantage': 0, 'username': 'B0' },
          { 'token': '03aA1', 'advantage': 0, 'username': 'B1' }
        ]
      ],
      [
        // Finale: nothing to do
      ]
    ];
