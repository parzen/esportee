...
class StageConfig {
    constructor(model) {
        ...
        if (model !== undefined) {
            ...
            this.nrLocations = model.nrLocations;
    ...
    scheduleMatches(matchArray, groups, nrGroups, leg, round, phase) {
        ...
        let matchesPossible = this.nrLocations;
        let nextGrpStart = 0;
        for (; round < nrMatchesTotal; round++) {
            ...
            while (grpScanned < nrGroups) {
                ...
                if (nrMatchesLeft > 0) {
                    ...
                    if (nrMatchesInGroup > 0) {
                        let result2 = this.addMatchesToRound(matchesInGroup, nrMatchesInGroup, matchesPossible, matchesInRound, nrMatchesInRound, matchArray, grp, this.nrLocations, round, leg, phase);
            ...
            if (matchesPossible > 0) {
                // TournamentCreator.Logger.info("Warning: %d empty location(s) during round! Perhaps not optimal scheduled!\n",matchesPossible);
            }
            matchesPossible = this.nrLocations;
        }
            ...
            if (onlyOnce) {
                for (let j = 0; j < this.matches[i].opponents.length; j++) {
                    ...
                    if (pseudoTeamNames) {
                        ...
                        if (pseudoTeams[idx].indexOf(token) <= -1) {
                        ...
                        if (array[idx].item.indexOf(token) <= -1) {
    ...
    addMatchesToRound(matchesInGroup, nrMatchesInGroup, matchesPossible, matchesInRound, nrMatchesInRound, matchArray, grp, nrLocations, round, leg, phase) {
        ...
        for (let m = 0; m < lenMatchesInGroupArray; m++) {
            ...
            let location = nrLocations - matchesPossible;
            if (wildcardMatch) {
            ...
            scheduledMatchesForThisRound.push({
                ...
                bracket: grp,
                round: round,
                opponents: [{ token: newTeamHome.token, score: 0, advantage: 1, user: null, resultApprovedTimestamp: 0 },
                    { token: newTeamAway.token, score: 0, advantage: 0, user: null, resultApprovedTimestamp: 0 }],
                leg: leg,
                phase: phase
            });
            if (!wildcardMatch) {
