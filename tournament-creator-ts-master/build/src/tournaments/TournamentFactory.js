...
class TournamentFactory {
    static createTournament(userInput, dummyParticipants) {
        ...
        for (let stageConfig of userInput.stageConfigs) {
            // clean stage
            stageConfig.clean();
            TournamentCreator_1.TournamentCreator.Logger.info("Create " + stageConfig.stageType + "-tournament with args:");
            ...
            switch (stageConfig.stageType) {
                case "GROUP":
                    ...
                    stageConfig.matches = StageFactory_1.StageFactory.createGroupStage(stageConfig, dummyParticipants).matches;
                    stageConfig.name = "Group Stage";
                    break;
                case "SINGLE_ELIMINATION":
                    // Create new single-elimination tournament
                    stageConfig.matches = StageFactory_1.StageFactory.createSingleEliminationStage(stageConfig, dummyParticipants).matches;
