import {LoggerFacade} from "./utils/LoggerFacade";
...
import {searchMatchesRO} from "./interfaces/IReturnObjects";
export class TournamentCreator {
    public static Logger: LoggerFacade = new LoggerFacade();

    public constructor(logger?: ILoggerService) {
        TournamentCreator.Logger.externalLogger = logger;
    }

    public static searchMatches(nrMemberInGroup: number, grp: number, matchArray: [[IMatchToken[]]], startTeam: IMatchToken, matchesInGroup: [[IMatchToken[]]], nrMatchesInGroup: number): searchMatchesRO {
        let nrMatchesNeeded = nrMemberInGroup / 2;
        let lenMatchesArray = matchArray[grp].length;
        let skipMatch: [IMatchToken[]] = [[]];
        ...
        let skippedArray = new Array<number>(nrMatchesNeeded);
    ...
    public static validateTournamentArgs(nrGroups: number, nrParticipants: number, legs: number, nrVenues: number, onlineTournament: boolean): ITournamentValidationError {
        let result: ITournamentValidationError = {errorCount:0, error: {key: null, error: null}};
