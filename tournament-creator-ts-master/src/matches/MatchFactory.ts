...
import {MatchToken} from "../matchtokens/MatchToken";
import {IMatchToken} from "../interfaces/IMatchToken";
export class MatchFactory {
    static EMPTY_ENTRY: IMatchToken = {token: "empty", score: -1, advantage: 0, resultApprovedTimestamp: null, user: {id: -1, username: "empty"}, team: {id: -1, name: "empty"}};

    public static createMatches(participants: IMatchToken[], stageOrder: number, nrGroups: number, nrParticipants: number, dummyParticipants: boolean, phase: number, leg: number): MatchGroupArray {
        let output: MatchGroupArray = {
            matchArray: [[[]]],
            groups: [[]]
        };
        ...
        for (let grp = 0; grp < nrGroups; grp++) {
            ...
            if(!output.groups[grp]) output.groups[grp] = [];
            ...
            for (let j = 0; j < nrMatches; j++) {
                if (!output.matchArray[grp]) output.matchArray[grp] = [[]];
    ...
    public static createTokens(nrParticipants: number, dummyParticipants: boolean, stageOrder: number, phase: number, group: number): MatchToken[] {
        let tokens: MatchToken[] = [];
        for (let i = 0; i < nrParticipants; i++) {
            let participant = new MatchToken();
