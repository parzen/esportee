...
import {TournamentConfig} from "../tournaments/TournamentConfig";
...
export class TcUtils {
    static getSortedTable(matches: IMatch[], sortRule: sortRule[], id: number): tableEntry[] {
        let sortedTable: tableEntry[] = [];
        let unsortedTable: tableEntry[] = [];
    ...
    static getSeTitle(maxPhase: number, phase: number, thirdPlaceMatch: boolean): string {
        let title = "";
        let idx = 1;
    ...
    static setName(table: tableEntry[], matches: IMatch[]) {
    ...
    static sortTable(table: tableEntry[], matches: IMatch[], sortRuleArray: sortRule[], id: number, sortRuleIdx: number): tableEntry[] {
        if (sortRuleIdx === 0) {
    ...
    static getTable(allMatches: IMatch[], participantFilter: string[]): tableEntry[] {
        let unsortedTable: tableEntry[] = [];
        ...
        for (let match of allMatches) {
            if (match.status !== MatchState.FINISHED) continue;
    ...
    static getParticipants(table: tableEntry[]): string[] {
        let participants: string[] = [];
        for (let i = 0; i < table.length; i++) {
    ...
    static updateNextStage(tournament: TournamentConfig, currentBracket: number, currentPhase: number, currentStageOrder: number): IMatchToken[] {
        ...
        if (matches == null || matches.length == 0) {
            if (verbose) console.log("No matches in stage/phase with group:"+currentBracket+" and phase:"+currentPhase+"-> do nothing");
            return iMatchTokens;
        }

        if (!TcUtils.allFinished(matches)) {
            if (verbose) console.log("All matches not yet finished in stage:"+currentStageOrder+" group:"+currentBracket+" and phase:"+currentPhase+" -> do nothing");
            return iMatchTokens;
        }
        if (verbose) console.log("All matches finished in group: "+currentBracket+" and phase:"+currentPhase+"!");
        ...
        if (currentPhase < maxPhase) {
            ...
            if (currentStage.thirdPlaceMatch && currentPhase == maxPhase - 1) {
                ...
                if (verbose) console.log("Es gibt eine höhere Phase!")
                let nextPhases = [];
                ...
                for (let i = 0; i < nextPhases.length; i++) {
                    ...
                    if (verbose) console.log("classified: " + JSON.stringify(classified));

                    let participant;
                    if (classified.length < 1) {
                        if (verbose) console.log("Not enough participants in group, fill up with empty ones!");
                        participant = null;
                    } else {
                        participant = {
                            id: classified[0].id,
            ...
            if (nextStages.length > 0) {
                if (verbose) console.log("Es gibt eine höhere Stage!");
                let nextStage = nextStages[0];

                if (verbose) console.log("currentStage.nrGroups " + currentStage.nrGroups + " -> " + nextStage.nrGroups + " nextStage.nrGroups");
                ...
                if (classifiedPerGroup / 2 /* classified As Home Team */ > nrOfCluster) {
                    special = 1;
                    if (verbose) console.log("Special");
                }
                if (nrClassifiedGroups >= 4 && classifiedPerGroup > nrOfCluster) {
                    ...
                    specialHop = 1;
                    if (verbose) console.log("Specialhop");
                }
                ...
                for (let team of classified) {
                    ...
                    if (verbose) console.log("[" + group + "][" + advantage + "] -> " + team.name + " (id: " + team.id + ")");
    ...
    public static allFinished(matches: IMatch[]): boolean {
        for (let match of matches) {
            if (match.status != MatchState.FINISHED && match.status != MatchState.WILDCARD) {
                return false;
            }
    ...
    public static checkNextStageCombination(currentStage: IStageConfig, nextStage: IStageConfig): boolean {
        ...
        if (!(currentStage.stageType == StageType.GROUP && nextStage.stageType == StageType.SINGLE_ELIMINATION)) {
            validCombination = false;
        }
    ...
    private static checkCombination(currentNrGroups: number, nextNrGroups: number): boolean {
    ...
    private static getOppositeIdx(idx, lastIdx) {
        return (lastIdx - 1) - idx;
    }

    private static getGroupIdx(group, clusterSize) {
        let off = Math.floor(group / clusterSize);
        return group - off * clusterSize;
    }

    private static mod(n, m) {
        return ((n % m) + m) % m;
    }

    private static isEven(x): boolean {
    ...
    private static hopsize(clusterSize, bracket, center, special) {
        let i;
    ...
    private static neighborBracket(bracket, nrClassifiedGroups) {
        if (nrClassifiedGroups === 1) {
            return bracket;
        } else {
            if (TcUtils.isEven(bracket)) {
    ...
    private static getClusterIdx(group, clusterSize) {
