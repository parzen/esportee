...
import {addMatchesToRoundRO, previewArray, searchMatchesRO} from "../interfaces/IReturnObjects";

import {sortRule, tableEntry, TcUtils} from "../utils/TcUtils";
import {IMatch, MatchState} from "../interfaces/IMatch";
import {IMatchToken} from "../interfaces/IMatchToken";
import {ITournamentValidationError} from "../interfaces/ITournamentValidationError";
...
export class StageConfig implements IStageConfig {
    ...
    constructor(model?: IStageConfig) {
        this.matches = [];
        if (model !== undefined) {
            this.id = model.id;
    ...
    public scheduleMatches(matchArray: [[IMatchToken[]]], groups: [IMatchToken[]], nrGroups: number, leg: number, round: number, phase: number, venues: Venue[], onlineTournament: boolean): schedulePlan {
        let scheduledMatches: IMatch[] = [];
        ...
        let nrGrpMember: number[] = [];
        for (let i = 0; i < nrGroups; i++) {
            nrGrpMember[i] = groups[i].length;
            if ((nrGrpMember[i] % 2) !== 0) {
                TournamentCreator.Logger.error("Error: nrMemberInGroup is odd, this should not happen!!\n");
            }
        }

        // Array containing the next start team
        let startTeamArray: IMatchToken[] = [];
        for (let i = 0; i < nrGroups; i++) {
            startTeamArray[i] = groups[i][0];
        }

        let matchesInGroup: [[IMatchToken[]]] = [[[]]];
        for (let i = 0; i < nrGroups; i++) {
            matchesInGroup[i] = [[]];
        ...
        let matchesInRound: [IMatchToken[]] = [[]];
        ...
        for (; round<nrMatchesTotal; round++) {
            ...
            while (grpScanned < nrGroups) {
                ...
                if (nrMatchesLeft > 0) {
                    ...
                    if (nrMatchesInGroup === 0) {
                        ...
                        let result2: searchMatchesRO = TournamentCreator.searchMatches(nrGrpMember[grp],
                            grp,
                            matchArray,
                            startTeamArray[grp],
                            matchesInGroup,
                    ...
                    if (nrMatchesInGroup > 0) {
                        let result2: addMatchesToRoundRO = this.addMatchesToRound(matchesInGroup,
                            nrMatchesInGroup,
                            matchesPossible,
                            matchesInRound,
                            nrMatchesInRound,
                        ...
                        if (result2.errors.errorCount > 0) {
                            TournamentCreator.Logger.error("Error: addMatchesToRound() aborts with error! [" + result2.errors.error.error + "]\n");
            ...
            if (matchesPossible > 0 && nrMatchesInRound === 0) {
                TournamentCreator.Logger.error("Error: No matches in round " + round + ", this should not happen!!\n");
                result.errors.errorCount++;
    ...
    public getSortedTableForBracket(bracket: number, sortRule: sortRule[]): tableEntry[] {
        let matches: IMatch[] = this.matches.filter(function (item) {
            return item.bracket === bracket;
        });

        return TcUtils.getSortedTable(matches, sortRule, this.id);
    }

    // get sorted table for all groups for current stage: nrGroups x participantsInGroup Array
    public getSortedTableForAllBrackets(sortRule: sortRule[]): [tableEntry[]] {
    ...
    public getPreviewArray(pseudoTeamNames: boolean): previewArray[] {
        let array: previewArray[] = []; // [{name:"", item: []}]
        let name = "";
        let idx = 0;
        let onlyOnce = 0;
        let firstPhase = -1;
        ...
        for (let i = 0; i < this.matches.length; i++) {
            if (this.stageType == "GROUP") {
                ...
                if (firstPhase == -1) firstPhase = idx;

                name = TcUtils.getSeTitle(maxPhase, this.matches[i].phase, this.thirdPlaceMatch);
                onlyOnce = 0;
            }
            ...
            if (onlyOnce) {
                for (let j = 0; j < this.matches[i].opponents.length; j++) {
                    ...
                    if (pseudoTeamNames) {
                        ...
                        if (pseudoTeams[idx].indexOf(token) <= -1 && token != MatchFactory.EMPTY_ENTRY.token) {
                            pseudoTeams[idx].push(token);
                            array[idx].item.push("Team " + String.fromCharCode(65 + teamNameIdx));
                            teamNameIdx++;
                        }
                    } else {
                        if (array[idx].item.indexOf(token) <= -1 && token != MatchFactory.EMPTY_ENTRY.token) {
    ...
    addMatchesToRound(matchesInGroup: [[IMatchToken[]]], nrMatchesInGroup: number, matchesPossible: number, matchesInRound: [IMatchToken[]], nrMatchesInRound: number, matchArray: [[IMatchToken[]]], grp: number, venues: Venue[], round: number, leg: number, phase: number, onlineTournament: boolean) {
        let scheduledMatchesForThisRound: IMatch[] = [];
        let result: addMatchesToRoundRO = {
            errors: {errorCount: 0, error: {key: "", error: ""}},
        ...
        for (let m = 0; m < lenMatchesInGroupArray; m++) {
            ...
            if (newTeamHome == MatchFactory.EMPTY_ENTRY || newTeamAway == MatchFactory.EMPTY_ENTRY) {
                wildcardMatch = true;
            }
            ...
            if (beforeNrMatchesInGroupLeft === nrMatchesInGroup) {
                TournamentCreator.Logger.error('Error: IMatch not found, this should not happen!!\n');
                result.errors.errorCount++;
                result.errors.error.key = "internal_server_error";
    ...
    resultHasError(result, stageType): number {
        if (result.error != null && result.error.errorCount !== 0) {
            TournamentCreator.Logger.error("Error: '" + stageType + "'-creation aborts with error: " + result.error.error + "!\n");
            return 1;
        } else {
            return 0;
        }
